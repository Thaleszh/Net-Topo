...
int csc::get(int i, int j) {		// does not verify if j,i < _nodes, but only returns -1
...
std::list<const node*> csc::path_find(int a, int b) {
	...
	if (a != b) {
		...
		// possibility that visited is not being used, check after implementing memoization
