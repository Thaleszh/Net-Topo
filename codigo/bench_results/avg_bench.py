...
for name in ["mesh2D_16", "tree_7"]:
	init_memoi[name] = (pd.read_csv("init." + name + ".txt", sep=" ", header=None)).to_numpy().astype(int)
	load[name] = (pd.read_csv("load." + name + ".txt", sep=" ", header=None)).to_numpy().astype(int)
...
print(np.sum(dist_1_fill["mesh3D_8"]))
for name in ["mesh3D_8", "tree_10"]:
