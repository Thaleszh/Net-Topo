...
#include "elements.h"
#include "ckheap.h"
#include "myNeighborLB.h"

CMI_EXTERNC_VARIABLE int quietModeRequested;

CreateLBFunc_Def(myNeighborLB, "The neighborhood load balancer")

myNeighborLB::myNeighborLB(const CkLBOptions &opt):CBase_myNeighborLB(opt)
{
  lbname = "myNeighborLB";
  if (CkMyPe() == 0 && !quietModeRequested)
    CkPrintf("CharmLB> myNeighborLB created.\n");
}

LBMigrateMsg* myNeighborLB::Strategy(myNborBaseLB::LDStats* stats, int n_nbrs)
{
#if CMK_LBDB_ON
  ...
  if (_lb_args.debug()>1)
    CmiPrintf("Starting migration. %d Neighbors.\n", n_nbrs);
